...
incomparable      <- readRDS("/Users/Lukas/repos/incomparable/data/incomparable_master.rds") %>% 
                     filter(podcast == "The Incomparable")
incomparable_wide <- readRDS("/Users/Lukas/repos/incomparable/data/incomparable_master_wide.rds") %>% 
                     filter(podcast == "The Incomparable")
...
incomparable_wide %>%
  ...
  {
  ggplot(data = ., aes(x = number_total, y = duration, colour = host)) +
  geom_point(size = 2, color = "black") +
  geom_point() +
  geom_label_repel(data = filter(., duration > 150 | duration < 30), 
                   aes(label = paste0("#", number)), size = 3) + 
  labs(title = "The Incomparable Podcast\nShow Duration",
  ...
  theme_breaks_y +
  theme_breaks_x +
  guides(colour = guide_legend(override.aes = list(fill = host_colors)))
  }
```
