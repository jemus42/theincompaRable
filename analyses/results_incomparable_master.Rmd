...
In `r year(now())-1`, there were **`r incomparable_master_wide %>% filter(year == year(now())-1) %>% nrow()`** episodes published, and based on the average release frequencies, there will probably be **`r projection`** episodes in `r year(now())`.
